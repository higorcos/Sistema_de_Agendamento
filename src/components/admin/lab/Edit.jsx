...
export default function AdminNews(props) {
  ...
  useEffect(() =>{
  api.get(`/news/edit/show/${slugNoticia}/`).then((response)=>{
  ...
  const updateImgMain=(nameRemove)=>{
    ...
    api.post(`/news/update/img/${idNoticia}`, dataJson, headers).then((response)=>{
      const result = response.data
      if(result.err){
        console.log("ERRO")
  ...
  api.get(`/news/edit/img/${idNoticia}/${item}`).then((response)=>{
    const result = response.data
    if(result.err){
      console.log("ERRO")
  ...
  await api.post(`/news/update/${slugNoticia}`,dataJson, headers).then((res)=>{
    alert('Atualizado')
