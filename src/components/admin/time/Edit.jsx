...
export default function EditTime(props) {
  ...
  useEffect(() => {
    ...
    api.get(`/time/edit/${idTime}`).then((response)=>{
      const result = response.data
      if(result.err){
      }else{
      setVisibilidade(result.dados[0].visibilidade);
      setName(result.dados[0].name);
      setTurno(result.dados[0].turno);
      set_horario_inicial(result.dados[0].horario_inicial);
      set_horario_final(result.dados[0].horario_final);
...
const click = (value)=>{
  // document.getElementById("id-switch-visibilidade-time").click()
  console.log(value)
}
